...
Конвейер вызывает term_stats для нескольких кандидатов параллельно (STATS_CONCURRENCY), поэтому
пауза общая на процесс (как RateLimiter у arXiv), а не только внутри одного news_by_year.
"""
...
_rate_limiter = RateLimiter(interval_s=PAUSE_S)
