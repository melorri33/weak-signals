...
| `GET /search/{run_id}/evidence` | признаки и публикации по годам для всех кандидатов, включая отсеянных: из них веб-интерфейс строит карту сигналов и графики динамики. Пишутся в `data/search_evidence`; у прогонов, сделанных раньше, их нет (`404`) |
...
| Экран | Что на нём |
| --- | --- |
| Главная | строка запроса, примеры из ТЗ, карта последнего поиска, последние поиски карточками с топ-3, состояние модели |
| Поиск | пока идёт — шаги конвейера, поисковые фразы, счётчики и кандидаты на карте; потом воронка отсева, карта сигналов, топ-15 с динамикой публикаций, отсеянное с причиной, все кандидаты, журнал моделей |
| Инсайт | отчёт по сигналу: описание, преимущество, кейс, оценки в отчётах, почему слабый сигнал, динамика публикаций, вклад признаков (SHAP), источники; листается стрелками ← →, печать в PDF |

**Карта сигналов**: по горизонтали — сколько всего публикаций (логарифм), по вертикали — доля вышедших за последние три года. Слабые сигналы — слева вверху. Строится из `GET /search/{run_id}/evidence`, поэтому у прогонов, сделанных до её появления, карты нет.

**Режим показа для жюри**: кнопка в шапке или `?show=1` в адресе — всё крупнее, служебные детали скрыты.
