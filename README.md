...
**Где живёт модель.** Контейнеру нужен адрес модели, и `localhost` внутри контейнера — это сам
контейнер, поэтому для Docker есть отдельная переменная `OLLAMA_URL_IN_DOCKER`:
...
| В контейнере `llm_available: false`, а на хосте модель работает | в `.env` не задан `OLLAMA_URL_IN_DOCKER` (внутри контейнера `localhost` — это он сам) |
...
`extract_candidates` на пачке из 8 документов — **143 с** на `qwen3:8b`. В прогоне таких пачек
десятки (до 500 документов по 8), то есть часы против бюджета шага в 180 с. Что происходит на самом
деле: шаг не успевает, срабатывает запасная нарезка названий из заголовков, и в топ-15 попадают
«TechCrunch Mobility» вместо технологий. На видеокарте (RTX 3060, 14B) полный прогон — 7,5 минуты.

Вывод: без видеокарты прогон делаем на облачной модели (`LLM_PROVIDER=yandexgpt`), а бюджеты шагов
в `src/pipeline/run.py` рассчитаны на видеокарту — для облака их ещё будем перемерять.
