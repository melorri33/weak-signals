...
# Термин может быть разорван переносом строки, разделён дефисами внутри адреса или расставлен
# по markdown-разметке. Поэтому перед поиском схлопываем всё это в обычные пробелы — иначе
# нарушение прячется от проверки, оставаясь нарушением.
#
# Примеры сюда подставлять нельзя: пример нарушения сам становится нарушением.
# Так уже случалось трижды, последний раз — в этом самом файле.
